...
@section('content')
    <form action="{{ route('register') }}" method="post" autocomplete="off" class="registration">
        ...
        <input class="{{ $errors->has('password') ? 'error__input' : 'registration__input'}}" name="password" id="password" type="password" placeholder="Пароль (минимум 8 символов)">
        ...
        <div class="registration_checkbox flex">
            <input class="checkbox" required type="checkbox">
            <label>Я принимаю условия <a href="{{ asset('docs/09_06_24_Пользовательское_соглашение_Техно_Портал.docx') }}">Пользовательского соглашения</a></label>
        </div>
